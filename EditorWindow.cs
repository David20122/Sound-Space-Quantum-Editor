...
namespace Sound_Space_Editor
{
	class EditorWindow : GameWindow
	{
		...
		public EditorWindow(long offset) : base(1080, 600, new GraphicsMode(32, 8, 0, 8), "Sound Space Map Editor v1.4.3f")
		{
			Instance = this;
		...
		public bool WillClose()
		{
			if (!_saved && _soundId != -1)
			{
				...
				var r = MessageBox.Show("save your funny map?", "Close", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning);
		...
		private void OnDraggingGridNote(Point pos)
		{
				...
				if (GuiScreen is GuiScreenEditor editor && _draggedNotes.FirstOrDefault() is Note note)
				{
					...
					if (GuiScreen is GuiScreenEditor gse)
					{
						if (gse.Quantum.Toggle)
						{
							...
							if (QGMnewX < -0.850f || QGMnewX > 2.850f || QGMnewY < -0.850f || QGMnewY > 2.850f)
							{
								return;
							}
			...
			} catch
			{
				if (GuiScreen is GuiScreenEditor editor)
				{
					MessageBox.Show("yoo the editor would crash beacuse of\n \nentering letters into snapping box\nor\nsomething gone wrong while dragging\n\nsave your map pls", "error :sob:", MessageBoxButtons.OK,
					MessageBoxIcon.Error);
					editor.ShowToast("saving map", Color.FromArgb(255, 255, 255));
					PromptSave();
				}
			}
		}
